...
o programa deverá ser encerrado, exibindo o relatório do dia, que conterá a quantidade e o valor total de prestações
...
valores = list()
while True:
    valor = float(input('Valor da prestação[0-Encerra]: '))
    if valor == 0: break
    dias = int(input('Dias de atraso: '))
    valor = valorPagamento(valor, dias)
    valores.append(valor)
    print(f'Valor a pagar: R${valor}')
print(f'Relatorio do dia: R${sum(valores):.2f} reais pagos hoje')