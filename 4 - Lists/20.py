...
while True:
    valor = float(input('Salario R$[0-Encerra]: '))
    if valor == 0:
        x = 0
        break
    salarios.append(valor)
    abono.append((salarios[x] / 100) * 20)
    if  abono[x] < 100:
        abono[x] = 100
    
    x+=1

print(f'Salario  -    Abono')
for i, j in zip(salarios, abono):
    print(f'R${i} - R$  {j}')
    if j == 100:
        x+= 1
print(f'Foram processados {len(salarios)} colaboradores')
print(f'Total gasto com abonos: R${sum(abono):.2f}')
print(f'Valor minimo pago a {x} colaboradores')
print(f'Maior valor de abono pago: {max(abono)}')
