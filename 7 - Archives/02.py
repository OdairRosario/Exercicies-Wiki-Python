...
de uma função separada, que será chamada pelo programa principal. O cálculo do percentual de uso também deverá ser feito através
...
with open('2Usuarios.txt', 'r') as arquivo:
        for i,linha in enumerate(arquivo):
            dados['nomes'].append(linha[0:15])
            dados['armazenamento'].append(float(linha[15:]) / (1024*1024))

        for item in dados['armazenamento']:
            dados['total'] += item

        dados['medio'] = (dados['total'] / len(dados['nomes']))

        for item in dados['armazenamento']:
            porcentagem = (item * 100) / dados['total']
            dados['porcentagem'].append(porcentagem)

with open('2Relatorio.txt', 'w') as a:
    a.write('ACME Inc.               Uso do espaco em disco pelos usuarios\n')
    a.write('------------------------------------------------------------------------\n')
    a.write("Nr.  Usuario        Espaco utilizado     % do uso\n")
    
    for i in range(0,len(dados['nomes'])):
        a.write('\n{}    {}{:.2f} MB             {:.2f}%'.format(i+1, dados['nomes'][i], dados['armazenamento'][i], dados['porcentagem'][i]))
    a.write('\n\nEspaco total ocupado: {:.2f}\nEspaco medio ocupado: {:.2f}'.format(dados['total'],dados['medio']))